...
//	server->pollfd[server->count -1].fd = server->pollfd[index].fd;
	server->pollfd[index].fd = server->pollfd[server->count -1].fd;
	server->pollfd = (struct pollfd *) realloc(server->pollfd,
						(--server->count) * sizeof(struct pollfd));
...
//	recvcount = fscanf(in, "%d %s", &clientfd, cmdbuf);	/* can't get arguments sucn as 4 ls -l will not get -l */
	...
	fprintf(stdout, "%s: client fd %d, cmd %s(%d)\n", __func__, clientfd, cmdbuf, strlen(cmdbuf));
	...
	if (clientfd == 0 && !strcmp(" show clients\n", cmdbuf)) {
		...
		clientfd = -1;
		return ;
	} else if (clientfd == 0 && !strcmp(" exit\n", cmdbuf)) {
		fprintf(stdout, "want exit ???\n");
		exit_do_it = 1;	// exit do_it routine
		clientfd = -1;
		return ;
	} else if (clientfd == 0) {
		fprintf(out, "%s: for show clients info\n", __func__);
		fprintf(out, "%s: 0 show clients\n", __func__);
		fprintf(out, "%s: 0 exit\n", __func__);
		return ;
	} else {
		;
	}
...
client_t *accept_new_client(FILE *out, server_t *server)
{
	client_t *client = NULL;
...
void do_it(FILE *in, FILE *out, server_t *server, int timeout)
{
	...
	client_t	*new_client = NULL;
        ...
        for ( ; ; ) {
		if (exit_do_it) return ;
		if (new_client) {
			if (add_fd_to_pollfd(server, new_client->connectfd)) 
				fprintf(stdout, "add new client ok\n");
			else 
				fprintf(stderr, "add new client failed\n");	// just print the err info, and not remove new client from server's list
			new_client = NULL;
		}
                fprintf(out, "#");
			...
			new_client = accept_new_client(out, server);
                }
			...
			if (server->pollfd[i].fd < 0) continue;
                        if (server->pollfd[i].revents & POLLIN) {
							...
							recvbuf, 
							MAX_RECV_BUF)) {
					fprintf(stdout, "delete client from server\n");
					sleep(4);
					del_client_from_server(server, client);
					destroy_client(&client);
